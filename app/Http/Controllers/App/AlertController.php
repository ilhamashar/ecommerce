...
class AlertController extends Controller
{
   ...
   /* public function __construct()
	...
	public function createUserAlert($existUser){
		...
		$existUser[0]->app_name = $setting[0]->app_name;	
		
		if($alertSetting[0]->create_customer_email==1 and !empty($existUser[0]->customers_email_address)){					
			Mail::send('/mail/createAccount', ['userData' => $existUser], function($m) use ($existUser){
				$m->to($existUser[0]->customers_email_address)->subject(Lang::get("labels.WelcometoEcommerce"))->getSwiftMessage()
				->getHeaders()
		...
		if($alertSetting[0]->create_customer_notification==1){	
			...
			$message = Lang::get("labels.welcomeemailtext").$setting[0]->app_name;
	...
	public function orderAlert($ordersData){
		...
		//setting 
		$setting = $this->setting();
		$ordersData['app_name'] = $setting[0]->app_name;
		$ordersData['admin_email'] = $setting[0]->contact_us_email;	
		...
		if($alertSetting[0]->order_email==1){					
			...
			//customer email
			if(!empty($ordersData['orders_data'][0]->customers_email_address)){
				Mail::send('/mail/orderEmail', ['ordersData' => $ordersData], function($m) use ($ordersData){
		...
		if($alertSetting[0]->order_notification==1){	
			...
			$message = Lang::get("labels.OrderDetail").$setting[0]->app_name;
	...
	public function forgotPasswordAlert($existUser){
		...
		if($alertSetting[0]->forgot_email==1 and !empty($existUser[0]->customers_email_address)){	
			Mail::send('/mail/recoverPassword', ['existUser' => $existUser], function($m) use ($existUser){
				$m->to($existUser[0]->customers_email_address)->subject(Lang::get("labels.fogotPasswordEmailTitle"))->getSwiftMessage()
				->getHeaders()
				->addTextHeader('x-mailgun-native-send', 'true');	
			});
		}
